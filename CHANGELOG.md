...
- Figma → repo design-token pipeline (`lib/styles/scripts/figma/`): Figma Variables (colors, themes, layout, typography, motion) export to `figma-tokens.json`; `bun run figma:import` regenerates the source config and runs the WCAG contrast gate. Figma is now the source of truth for those token families; easings and breakpoints stay code-owned.
- Foundations canvas boards in the token Figma file — layout grid, motion durations, primitives, contrast pairs, UI specimens, and line-height specs, all property-bound to the synced variables — plus `surfaces-board.figma.js`, a repaint snippet for the derived (code-owned) surface/line preview board.
...
- Pre-commit no longer fails when every staged file is biome-ignored (e.g. `*.figma.js` snippets): the lefthook biome command now passes `--no-errors-on-unmatched`.
...
- The `cta` text style owns its vertical trim: `leadingTrim: CAP_HEIGHT` in Figma flows into the generated `cta` utility as `text-box-trim: both; text-box-edge: cap alphabetic`, so Figma and browser CTA text align by construction (previously trim existed only on the `button` element rule).
