...
- Palette source of truth is Figma Variables, snapshotted in `lib/styles/scripts/figma/figma-tokens.json` and imported by `bun run figma:import`, which regenerates `lib/styles/colors.ts` (plus `layout.mjs`, `typography.figma.ts`, `motion.ts` — all carry a `GENERATED` banner; never hand-edit them). The theme CSS is then generated by `bun run setup:styles`. Never hand-edit `lib/styles/css/tailwind.css` / `root.css`. Breakpoints and the `typography.ts` override shell stay code-owned. Workflow: `lib/styles/scripts/figma/README.md`.
